...
func (c *Config) AddFlags(fs *flag.FlagSet) {
	...
	fs.Var(newGroupByValue(&c.GroupBy, c.GroupBy), "group-by", "When --cpu-device-mode=grouped, sets the criteria for grouping CPUs. Can be set to 'socket', 'numanode', 'machine' (machine mode requires an external scheduler to include cpuset configuration in claim allocation results), or 'l3cache' (groups CPUs by L3 cache domain).")
...
func (v *groupByValue) Set(s string) error {
	if s != driver.GROUP_BY_SOCKET && s != driver.GROUP_BY_NUMA_NODE && s != driver.GROUP_BY_MACHINE && s != driver.GROUP_BY_L3_CACHE {
		return fmt.Errorf("invalid value: %q, must be %s, %s, %s or %s", s, driver.GROUP_BY_SOCKET, driver.GROUP_BY_NUMA_NODE, driver.GROUP_BY_MACHINE, driver.GROUP_BY_L3_CACHE)
	}
	*v.value = s
	return nil
}
